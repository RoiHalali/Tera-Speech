...
def Tongue_detect(img,tounge_state,state):
    ...
    #cv_print (blur,"blur")
    # blur = cv2.blur(img, (9,9))   #blur for decrease noise

    # kernel = np.array([[-1,-1,-1], [-1,9,-1], [-1,-1,-1]])
    # img_filter = cv2.filter2D(blur, -1, kernel)

    hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV) #BGR to hsv
...
def tounge_down(img, factor, tounge_state):
    ...
    while cropp_flag == False:
        ...
        if len (cropp_img) != 0:
            ...
            return cropp_img, tongue_edge, loc    
...
def tounge(img, tounge_state):
    ...
    _, faces = detector.findFaceLandmark(img)        
    # error = cv2.imread('error.jpg')
    ...
    if faces != []:
        ...
        if loc != [] and loc != [0,0] :
            if (tounge_state == 'down'):
                ...
                ver_dis_pnt = cv2.circle(img, (loc[0], loc[1]), radius=3, color=(255, 255, 255),
                                          thickness=-1)  # draw points
                if (loc[1] < points[16,1]+10) and (num_labels == 2) :
                    cv2.putText(cropp,"down" ,(25,100),2,1,(255,255,255)) 
                    print('down')
            if (tounge_state == 'up'):
                distance = dist(points[11,0],points[11,1],loc[0],loc[1])
                if (loc[1] > points[11,1]+10) and (num_labels == 2) :
                    cv2.putText(cropp,"up" ,(25,100),2,1,(255,255,255)) 
                    print('up')
            if (tounge_state == 'right'):  #right side
                distance = dist(points[61,0],points[61,1],loc[0],loc[1])
                if (loc[1] < points[61,1]+10) and (num_labels == 2) :
                    cv2.putText(cropp,"right" ,(25,100),2,1,(255,255,255)) 
                    print('right')
            if (tounge_state == 'left'):  #left side
                distance = dist(points[291,0],points[291,1],loc[0],loc[1])
                if (loc[1] < points[291,1]+10) and (num_labels == 2) :
                    cv2.putText(cropp,"left" ,(25,100),2,1,(255,255,255)) 
                    print('left')

                      
        if num_labels == 2 and loc != [0,0]:
            if cleaned.any() != 0 and 'distance' in locals():
                # img_dots=print_dots(cropp,loc)
